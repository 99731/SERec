...
void mtx_fscanf(FILE* file, gsl_matrix* m) {
  ...
  for (i = 0; i < m->size1; i++) {
    for (j = 0; j < m->size2; j++) {
      if(1 != fscanf(file, "%lf", &x)){
	printf("reading matrix incorrectly..\n");
	}
      mset(m, i, j, x);
    }
  }
...
double normal_pdf(double x, double mean, double sigma,double mu){
    static const double inv_sqrt_2pi = 0.3989422804014327;
    double a = (x - mean) / sigma;

    //a =  inv_sqrt_2pi / sigma * std::exp(-0.5f * a * a );
    a =  inv_sqrt_2pi / sigma * fast_exp(-0.5f * a * a );
    return a; //a/(a + (1-mu)/mu);
}
